...
{
	float nominator = mix(2.0, 0.0001, vdot);
...
vec3 gltf_calculate_pbr(vec3 light_dir, vec3 light_intensity, vec3 view_dir, vec3 normal_dir, vec3 albedo, vec3 pbr)
{
	...
	vec3 F = f0 + (1 - f0) * pow(1 - v_dot_h, 5.0);

	vec3 f_diffuse = (1 - F) * (1 / PI) * c_diff;
	vec3 f_specular = F * gltf_microfacet_distribution(n_dot_h, alpha) * gltf_geometry_occlusion(n_dot_l, n_dot_v, alpha) / (4 * abs(n_dot_v) * abs(n_dot_l) + 0.001);
