...
layout(set = 0, binding = 0) uniform sampler2D luminance_in;
layout(set = 0, binding = 1) uniform Composite_params
{
	float exposure;
	float bloom_intensity;
} params;

layout(set = 0, binding = 2) uniform Exposure_result 
{
	float luminance;
	float prev_luminance;
} exposure;

layout(set = 0, binding = 3) uniform sampler2D bloom_in;
...
void main()
{
	...
	vec3 corrected_luminance = luminance / luminance_div;
	vec3 tonemapped_luminance = agx(corrected_luminance);
	vec3 gamma_corrected_luminance = pow(tonemapped_luminance, vec3(1.0 / gamma));
	color_out = vec4(gamma_corrected_luminance, 0.0);
}